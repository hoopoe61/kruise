...
func (h *UnitedObjectCreateUpdateHandler) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	appsv1alpha1.SetDefaultsUnitedObject(obj)
	obj.Status = appsv1alpha1.UnitedObjectStatus{}
	obj.Status.Instances = int32(len(obj.Spec.Instances))
	...
	if len(resp.Patches) > 0 {
		klog.V(5).Infof("Admit UnitedObject %s/%s patches: %v", obj.Namespace, obj.Name, util.DumpJSON(resp.Patches))
	}
	return resp
}
